...
def prepare_dual_map_data(
    ...
    """
    rootPath = Path(rootPath) if rootPath is not None else Path(os.path.dirname(os.getcwd()))
    html_path = "https://arquivos.lcqar.ufsc.br/rqar-national-guide-files/data/rep_espacial/inputs"
    muni_html = "https://arquivos.lcqar.ufsc.br/rqar-national-guide-files/data/shapefiles"
    
    stations_file = rootPath / "data/rep_espacial/outputs/estacoes_completa.parquet"
    industries_file = html_path.rstrip("/") + "/industrial_sites_20250902.gpkg"
    rep_csv = rootPath / "data/rep_espacial/outputs/rep_espacial.csv"
    roads_file = html_path.rstrip("/") + "/roads.parquet"
    ...
    muni_path = muni_html.rstrip("/") + "/BR_Municipios/BR_Municipios_2024.shp"
    try:
