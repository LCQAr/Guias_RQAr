...
Ajuste html_path para o diretório de saída no seu computador antes de executar o script
(é onde a tabela HTML gerada será salva localmente).
...
def land_use_table_interactive(
    ...
    img_path: str = "https://arquivos.lcqar.ufsc.br/rqar-national-guide-files/_static/bandeiras/",
    save_html: bool = True,
    ...
    # ======== Bandeiras em base64 ========
    def embed_flag(uf_value, img_path):
        url = f"{img_path}{uf_value}.png"
        try:
            resp = requests.get(url, timeout=10)
        except requests.RequestException as e:
            print(f"[bandeira] erro de conexão para {url}: {e}")
            return ""
        if resp.status_code == 200 and resp.content:
            b64 = base64.b64encode(resp.content).decode("utf-8")
            return f'<img src="data:image/png;base64,{b64}" width="25">'
        else:
            print(f"[bandeira] falhou ({resp.status_code}) para {url}")
            return ""

    df["Bandeira"] = df["UF"].apply(lambda uf: embed_flag(uf, img_path))
