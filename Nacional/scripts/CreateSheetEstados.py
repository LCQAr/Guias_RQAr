...
Este script padroniza os dados brutos de monitoramento de qualidade do ar
de cada unidade federativa (UF), unificando formatos, colunas e unidades
diferentes em uma estrutura única. Aplica os critérios de QAQC (controle
de qualidade), identifica os poluentes monitorados por estação e gera os
arquivos padronizados na pasta MQAr, organizados por poluente.
...
# muda o diretorio padrão (alterar)
...
"""
    ...
    os arquivos de cada estação existentes em cada subpasta. 
"""
    #ALTERAR
...
A função depende do arquivo que associa as UFs aos seus respectivos códigos IBGE -> dicionarios/IBGE_UFS_CODIGOS.csv
...
def ug_to_ppm(df):
"""
A função ug_to_ppm tem como objetivo padronizar a unidade de concentração dos dados para ppm para CO
"""
    # conversões
                    ...
                    for pol in poluentes:
                        ...
                        if pol in tabela_pols['POLUENTE'].values:
                           ...
                           # Padroniza 3 digitos 
                            ...
                            df_pol = df_pol[cols]
                            # Define DATETIME como índice novamente
            ...
            for est in estacoes[1:]:# A primeira coluna contém DATETIME, as demais correspondem às estações, portanto, o for inicia na segunda coluna
                ...
                for pol in poluentes[1:]:
                    ...
                    idx = df_est_pol.apply(lambda row: row.astype(str).str.contains("Flag").any(), axis=1).idxmax() # Como a linha contendo flag é a última linha antes dos valores medidos por padrão do estado 
                                                                                                                    #  o for exclui as linhas apartir do índice encontrado, reiniciando o index 
...
def rectify_PR(path):
    ...
    for ano in estacoes_por_ano.keys():
        ...
        for estacao in estacoes_por_ano[ano].keys():
            ...
            # Verifica os nomes das colunas DATETIME, para aplicar as funções de correção
    ...
    # Cria um dicionário que relaciona os anos as estações e os poluentes medidos para cada ano
    pol_estacao_ano = {}
    ...
    for ano in estacoes_por_ano.keys():
        ...
        pol_estacao_ano[ano] = {}
        # Passa por cada estação
        for estacao in estacoes_por_ano[ano]:
            # Cria a estrutura do dicionário Ano -> estação -> DataFrame
            pol_estacao_ano[ano][estacao] = {}
            # Verifica os poluentes da tabela de poluente medidos por Paraná, e se a estação mede esse poluente
            for pol in df_col_pols['col_pr']:
    
                if pol in estacoes_por_ano[ano][estacao].columns and 'UNIDADE' not in pol:
                    
                    # Encontra o nome do poluente padronizado    
                    poluente_mma = df_col_pols.loc[df_col_pols['col_pr'] == pol, 'col_mma'].iloc[0]

                    # Cria a coluna QAQC_INTERNO
                    qaqc_interno = np.nan

                    # Extraí a unidade do parênteses 
                    if '(' in pol:
                        unidade = pol.split('(')[-1][:-1]
                        print(pol)            
                        #print(unidade)

                        # Mantêm somente as colunas desejadas
                        ...
                        # Renomeia a coluna VALOR
                        df.rename(columns={pol: 'VALOR'}, inplace=True)

                    # Se existir uma coluna com o padrão POLUENTE_UNIDADE, renomeia essa coluna para UNIDADE 
                    ...
                    else: # Caso não exista, cria-se uma coluna UNIDADE com NaN
    ...
    pol_estacao = {}
    # O loop passa pelo ano, estação e poluente e adiciona o Datafreme daquele ano à lista: estação -> poluente -> DataFrame do ano
    ...
    estacoes_finais = {}
    # Junta todos os DataFrames daquele poluente e daquela estação em um unico DataFrame
    for estacao, poluentes in pol_estacao.items():
        for poluente, lista_dfs in poluentes.items():
            ...
            # Padroniza valores númericos para ponto decimal
            df_concat['VALOR'] = (
                df_concat['VALOR']
                .astype(str)           
            ...
            # Quando existem mais de um registro para uma data, faz a média do valor medido.
            df_media = (
                df_concat.groupby(['ANO', 'MES', 'DIA', 'HORA', 'UNIDADE'], as_index=False)['VALOR']
                  .mean()
            )
            # Cria o DATETIME a partir das colunas ANO, MES, DIA e HORA
            ...
            # Remove as colunas ano, mês, dia e hora originais do dataframe
            df_media = df_media.drop(columns=['ANO', 'MES', 'DIA', 'HORA'])
            # Tranforma o DATETIME em índice
            df_media = df_media.set_index("DATETIME")
            # Ordena cronologiamente
            df_media = df_media.sort_index()
            # Cria uma sequência com todos os DATETIMES esperados com base no primeiro e último registro
            lista_horas = pd.date_range(
                start=df_media.index.min(), 
                end=df_media.index.max(), 
                freq='H').strftime('%Y-%m-%d %H:%M:%S').tolist()
            # Compara as colunas existentes com a quantidade esperada e preenche as lacunas
            if len(lista_horas) != len(df):
                df_media = df_media.reindex(pd.DatetimeIndex(lista_horas))

            # Cria a coluna ANO, MES, DIA e HORA a partir do DATETIME
            ...
            # Cria uma chave com o nome da estação_poluente e adiciona ao dicionário
            estacoes_finais[estacao+'_'+poluente] = df_media

    primeiros_valores = {}

    # O loop percorre todas as chaves
    for chave, df in estacoes_finais.items():  

        # Verifica se o valor medido não for nulo
        if ~df['VALOR'].isna().all() and (df['VALOR'] > 0).any(): 
            # Encontra o primeiro registro com valor válido maior que zero e armazena a data
            linha_valida = df[df["VALOR"].notna() & (df["VALOR"] > 0)].iloc[0]
            primeiros_valores[chave] = linha_valida["DATETIME"]
    
    codigo_estacao_PR = {}
    # Percorre cada primeiro registro válido
    for chave in primeiros_valores.keys():
        # Extrai o nome da estação 
        station = chave.split('_')[0]
        # Extrai a primeira data válida daquela chave
        data = primeiros_valores[chave] 
        # Verifica se a estação está no dicionário
        if station in codigo_estacao_PR:
            # Se estiver e a data verificada é mais antiga que a registrada no dicionário, atualiza a data
            if data <= codigo_estacao_PR[station]:
                codigo_estacao_PR[station] = data
        else: # Se não estiver, é adicionado ao dicionário
            codigo_estacao_PR[station] = data
    ...
    codigo_estacao_PR = {}
    # Define o código da estação pela ordem da primeira data de monitoramento
    for i, (nome, ts) in enumerate(sorted_items, start=1):
        codigo = f"PR{i:04d}"
        codigo_estacao_PR[nome] = codigo
    # Para cada chave, retira-se o nome da estação e é associada ao seu ID_OEMA    
    for chave, df in estacoes_finais.items():
        
        estacao = codigo_estacao_PR[chave.split('_')[0]]
    
        if chave.split('_')[-1] != 'CS':
            # Encontra o código do poluente por meio da chave estação_poluente
            cod_pol = tabela_pols.loc[tabela_pols['NOME_PASTA'] == chave.split('_')[-1], 'COD_POLUENTE'].values[0]
            # Encontra qual pasta deverá ser salvo o DataFrame com base no código do poluente
            nome_pasta = tabela_pols.loc[tabela_pols['COD_POLUENTE'] == int(cod_pol), 'NOME_PASTA'].values[0]
            # Aplica o QAQC
...
df.to_csv('/home/nobre/Notebooks/RQAR_2025_book/data/MQAr/'+nome_pasta+'/'+estacao+'RA'+str(int(cod_pol)).zfill(3)+'.csv',index=False)
    # Cria um DataFrame contendo o ID_OEMA e o ID_MMA
    df_ids = pd.DataFrame({
...
def rectify_BA(path):
"""
A função padroniza os dados brutos de monitoramento da Bahia. 
Agrupa-se os dados de cada estação e poluente, indentifica a primeira data de monitoramento de cada estação para gerar o ID_MMA e aplica o QAQC.
"""
     # Define o caminho da pasta que contém os arquivos organizados por estação e poluente
    path = path+'/output_by_station_pollutant/'
     # Cria um dicionário para associar os DataFrames por estação e poluente
    dict_pols_stat = defaultdict(list)
    # Lista os arquivos existentes no caminho
    files = os.listdir(path)
        ...
        if 'csv' in item:
            ...
            if os.path.getsize(path+item) > 0:  # só tenta ler se não for vazio
                try:
                    ...
                    # Extrai o código do poluente lendo quarto elemento do nome do arquivo, de trás para frente, separado por "_"
                    pol = item.split('_')[-4]
                    
                    # Encontra o nome do poluente a partir do código
                    pol = tabela_pols.loc[tabela_pols['COD_POLUENTE'] == int(pol), 'NOME_PASTA'].values[0]
                    
                    # Extrai o nome da estação a partir do nome do arquivo
                    estacao = ''.join(item.split('_')[1:-4])
                    
                    # Converte o DATETIME para o formato de data e hora
                    df['DATETIME'] = pd.to_datetime(df["DATETIME"])

                    # Define o DATETIME como índice
                    df.index = df['DATETIME']

                    # Padroniza os nomes das colunas de valor medido para "VALOR" e QAQC_INTERNO, com o controle de qualidade da estação
                    df = df.rename(columns={'CONC':'VALOR','QAQC':'QAQC_INTERNO'})

                    # Associa a chave da estação ao seu DataFrame no dicionário
                ...
                # Informa quando o arquivo está vazio
                print(f"Arquivo vazio: {path+item}")
                df = pd.DataFrame()
    
    dict_formatado = {}
    # Percorre cada chave adicionada ao dicionário estação_poluente -> DataFrame
    for chave in dict_pols_stat.keys():

        # Obtém os DataFrames correspondentes à estação e ao poluente
        lista_dfs = dict_pols_stat[chave]

        # Concatena todos os Dataframes da estação_poluente em um único
        df = pd.concat(lista_dfs, ignore_index=True)
        
        # Garante que o DATETIME esteja no formato data e hora do Pandas
        df["DATETIME"] = pd.to_datetime(df["DATETIME"])
        ...
        # Cria uma sequência com todas as horas esperadas entre o primeiro e o último registro
        lista_horas = pd.date_range(
            start=df.index.min(), 
            end=df.index.max(), 
            freq='H').strftime('%Y-%m-%d %H:%M:%S').tolist()
        # Verifica se as horas existentes são iguais as horas esperadas e preenche os horários que não possuem registros
        if len(lista_horas) != len(df):
            df = df.reindex(pd.DatetimeIndex(lista_horas))

        # Padroniza os nomes das colunas e cria as colunas ANO, MES, DIA e HORA a partir do índice DATETIME
        df.insert(0, 'DATETIME', df.index)
        df.insert(1, 'ANO', df.index.year)
        df.insert(2, 'MES', df.index.month)
        df.insert(3, 'DIA', df.index.day)
        ...
        # Define a unidade de acordo com o poluente
        if chave.split('_')[-1] == 'CO':
            df['UNIDADE'] = 'ppm'
        else:
            df['UNIDADE'] = 'ug/m³'
        # Armazena o DataFrame já padronizado relacionando à chave
        dict_formatado[chave] = df
        
    primeiros_valores = {}
    # Percorre pelo dicionário contendo as chaves associadas aos DataFrames padronizados
    for chave, df in dict_formatado.items():
        # Verifica se existem valores válidos e maiores que zero
        if ~df['VALOR'].isna().all() and (df['VALOR'] > 0).any():
             # Obtém a primeira data em que houve uma medição válida maior que zero
            linha_valida = df[df["VALOR"].notna() & (df["VALOR"] > 0)].iloc[0]
            primeiros_valores[chave] = linha_valida["DATETIME"]
    
    codigo_estacao_BA = {}
    # O loop percorre a primeira data válida de cada estação e poluente
    for chave in primeiros_valores.keys():

        # Extrai o nome da estação
        station = chave.split('_')[0]
        # Armazena a primeira data válida daquela estação e poluente
        data = primeiros_valores[chave]

        # Verifica se a estação já foi adicionada ao dicionário
    ...
    # Ordena as estações pela primeira data de monitoramento
    sorted_items = sorted(
        codigo_estacao_BA.items(),
        key=lambda x: (x[1], x[0])
    )
    # Define o ID_MMA das estações de acordo com a ordem da primeira data de monitoramento
    codigo_estacao_BA = {}
    for i, (nome, ts) in enumerate(sorted_items, start=1):
        codigo = f"BA{i:04d}"
        codigo_estacao_BA[nome] = codigo
    # Percorre cada chave estação_poluente    
    for chave, df in dict_formatado.items():
        # Obtém o nome da estação
        estacao = codigo_estacao_BA[chave.split('_')[0]]
        # Extrai o nome do poluente da chave e verifica o código do poluente na tabela de códigos
        cod_pol = tabela_pols.loc[tabela_pols['POLUENTE'] == chave.split('_')[-1], 'COD_POLUENTE'].values[0]
        # Obtém a pasta do poluente que será salvo o DataFrame a partir do código do poluente
        nome_pasta = tabela_pols.loc[tabela_pols['COD_POLUENTE'] == int(cod_pol), 'NOME_PASTA'].values[0]
        # Aplica o QAQC
        df = create_QAQCMMA_VALOR(df,nome_pasta)
# Salva o .csv        
df.to_csv('/home/nobre/Notebooks/RQAR_2025_book/data/MQAr/'+nome_pasta+'/'+estacao+'ND'+str(int(cod_pol)).zfill(3)+'.csv',index=False)
    # Cria um novo DataFrame contendo o ID_OEMA e o ID_MMA
    df_ids = pd.DataFrame({
...
def rectify_MA(path):
"""
A função padroniza os dados brutos de monitoramento do Maranhão.
Agrupa-se os dados de cada estação e poluente, identifica a primeira data de monitoramento de cada estação para gerar o ID_MMA e aplica o QAQC.
"""
    # Define o caminho da pasta que contém os arquivos organizados por estação e poluente
    path = path+'/output_by_station_pollutant/'
    ...
    lista=[]
    # Define os poluentes que serão considerados
    lista_pols = ['MP25','MP10','CO','O3','NO2','SO2','PTS']
    # Define os termos utilizados para indentificar parte do nome dos arquivos
    lista_regex = ['OUT','CO','NO2','O3IQAR','UVIQAR','MP25',
                   'COIQAR','IQAR','MP10IQAR','MP25IQAR','O3',
                   'MP10','SO2','NO2IQAR','PTS','UV','IN']
    # junta os termos em uma lista separados por |
    lista_negada = "|".join(lista_regex)
    # Define o padrão utilizado para extrair o nome da estação do nome do arquivo
    ...
    for item in files:
        ...
        if 'csv' in item:
            ...
            # Extrai o código do poluente a partir do nome do arquivo
            pol = item.split('_')[-2]
            
             # Verifica se o poluente está na lista de poluentes considerados
            if pol in lista_pols:
                
                # Extrai o nome da estação utilizando a expressão padrão
                ...
                # Encontra o nome padronizado do poluente na tabela de códigos
                pol = tabela_pols.loc[tabela_pols['POLUENTE'] == pol, 'NOME_PASTA'].values[0]
                
                 # Extrai novamente o nome da estação
                estacao = estacao.group(1).replace("_", " ")
                
                # Converte o DATETIME para o formato de data e hora no padrão do Pandas
                df['DATETIME'] = pd.to_datetime(df["DATETIME"])
                
                # Define o DATETIME como índice
                df.index = df['DATETIME']

                # Padroniza o nome da coluna de valor medido para "VALOR"
                df = df.rename(columns={'CONC':'VALOR'})

                 # Remove a coluna COD, que não será utilizada
    ...
    for chave in dict_pols_stat.keys():
        ...
        # Verifica se as horas existentes são iguais às horas esperadas e preenche os horários que não possuem registros
        if len(lista_horas) != len(df):
            df = df.reindex(pd.DatetimeIndex(lista_horas))
    ...
    for chave in primeiros_valores.keys():
        ...
        # Extrai o nome da estação da chave
        station = chave.split('_')[0]
        # Armazena a primeira data válida daquela estação e poluente
        data = primeiros_valores[chave]
...
"""
Converte os valores de concentração dos poluentes SO2, NO2 e O3 de ppb para µg/m³.
"""
    if pol == 'so2':
...
def rectify_MT(path):
"""
A função padroniza os dados brutos de monitoramento de Mato Grosso.
Converte as unidades dos poluentes quando necessário, calcula as médias horárias,
organiza os dados por estação e poluente, identifica a primeira data de monitoramento 
de cada estação para gerar o ID_MMA e aplica o QAQC.
"""
    # Cria um dicionário para associar os DataFrames por estação e poluente
    dict_pols_stat = defaultdict(list)
    ...
    for item in files:
        ...
        if item != '.ipynb_checkpoints':
            ...
            # Remove a coluna com o nome da estação
            df = df.drop(columns=['Nome da estação'])

            # Obtém os poluentes presentes no arquivo
            lista_pols = set(df['Poluente'])

            # Percorre cada poluente
            for pol in lista_pols:

                # Mantém somente os dados do poluente atual
                df_pol = df[df["Poluente"] == pol]

                # Aplica a função que cnverte SO2, NO2 e O3 para µg/m³ quando necessário
                if pol in ['no2','so2','o3']:
                    df_pol = ppb_to_ug(df_pol,pol)
                # Agrupa os dados por ano, mês, dia, hora e unidade
                df_pol_hora = df_pol.groupby(["Ano", "Mes", "Dia", "Hora", "Unidade"])
                # Mantém somente os grupos que possuem pelo menos 9 registros 
                df_pol_hora = df_pol_hora.filter(lambda g: len(g) >= 9)

                # Calcula a média dos registros de cada hora
                df_pol = (
                ...
                df_pol = df_pol.rename(columns={'Ano':'ANO',
                                                ...
                                                'Unidade':'UNIDADE',
                                                'Valor':'VALOR'})
                # Converte as colunas de data e hora para valores numéricos
                for col in ["ANO", "MES", "DIA", "HORA"]:
                    df_pol[col] = pd.to_numeric(df_pol[col], errors="coerce").astype("Int64")
                # Associa o DataFrame à estação e ao poluente no dicionário
                dict_pols_stat[estacao+'_'+pol].append(df_pol)
    # Dicionário padronizando os nomes dos poluentes utilizados nos dados de Mato Grosso
    dict_pols_MT = {
        'co':'CO',
        'no2': 'NO2',
        'so2': 'SO2',
    ...
    for chave in dict_pols_stat.keys():
        ...
        # Concatena todos os DataFrames da estação_poluente
        df = pd.concat(lista_dfs, ignore_index=True)
        ...
        # Preenche os horários que não possuem registros
        if len(lista_horas) != len(df):
            df = df.reindex(pd.DatetimeIndex(lista_horas))

        # Cria a coluna DATETIME a partir do índice
        df['DATETIME'] = df.index

        # Mantém somente as colunas padronizadas
    ...
    # Percorre pelo dicionário contendo os DataFrames padronizados
    for chave, df in dict_formatado.items():
    ...
    for chave in primeiros_valores.keys():
        ...
        if station in codigo_estacao_MT:
            ...
            # Se estiver e a data verificada for mais antiga,
            # atualiza a data registrada
            if data <= codigo_estacao_MT[station]:
                codigo_estacao_MT[station] = data
                
        else: # Se não estiver, adiciona a estação ao dicionário
            codigo_estacao_MT[station] = data
    ...
    # Define o ID_MMA das estações pela ordem da primeira data de monitoramento
    for i, (nome, ts) in enumerate(sorted_items, start=1):
        codigo = f"MT{i:04d}"
 ...
 # Salva o .csv       
    ...
    # Substitui os nomes das estações pelos nomes padronizados
...
def fix_24h(row):
"""
Corrige registros de horário que apresentam 24:00, convertendo-os para
00:00 do dia seguinte.
"""
    # Verifica se o valor é uma string e começa com 24:
    if isinstance(row, str) and row.startswith("24:"):
        # substitui 24: por 00:
        new_str = row.replace("24:", "00:", 1)
        # converte para datetime
        dt = pd.to_datetime(new_str, errors="coerce")
        # Adiciona um dia ao registro
        if pd.notna(dt):
            dt += timedelta(days=1)
        return dt
    else: # Converte os demais registros diretamente para datetime
        return pd.to_datetime(row, errors="coerce")

def rectify_DF(path):
"""
A função padroniza os dados brutos de monitoramento do Distrito Federal.
Organiza os dados por estação e poluente, corrige registros de horário,
identifica a primeira data de monitoramento de cada estação para gerar o ID_MMA e aplica o QAQC.
"""
    # Define o caminho do arquivo com os dados de monitoramento de 2024
    path = path + 'Monitor Report 2024_FINAL.xlsx'

    # Lê o arquivo Excel
    df = pd.read_excel(path)

    # Preenche os valores ausentes da segunda linha utilizando o último valor válido
    df.iloc[1] = df.iloc[1].ffill()
    # Define os nomes dos poluentes presentes no arquivo
    poluentes = ['CO_ppm','NO2_ug/m3','NO_ug/m3','NOx_ug/m3','O3_ug/m3','PM10','PM25','PTS','SO2_ug/m3']
    # Cria um dicionário padronizando os nomes dos poluentes utilizados no arquivo
    dict_pols = {'CO_ppm':'CO',
                 'NO2_ug/m3':'NO2',
                 'NO_ug/m3':'NO',
                 'NOx_ug/m3':'NOX',
                 ...
                 'PTS':'PTS',
                 'SO2_ug/m3':'SO2'}
    # Define a segunda linha do arquivo como nome das colunas
    df.columns = df.iloc[1]

    # Reinicia o cabeçalho removendo as duas primeiras linhas
    df = df.drop(index=[0, 1]).reset_index(drop=True)

    # Padroniza o nome da coluna DATETIME
    df = df.rename(columns={'Date Time':'DATETIME'})

    # Obtém os nomes das estações
    estacoes = set(df.columns[1:])

    # Cria um dicionário para associar os DataFrames por estação e poluente
    dict_pols_stat = defaultdict(list)

    # O loop percorre cada estação
    for estacao in estacoes:
         # cria um DataFrame somente as colunas de data/hora e da estação
        df_estacao = df[["DATETIME",estacao]]
        
        # Utiliza a primeira linha da estação como nome das colunas dos poluentes
        df_estacao.columns = [df_estacao.columns.tolist()[0]] + df_estacao.iloc[0, 1:].tolist()
        
        # Remove a linha utilizada para definir os nomes dos poluentes
        df_estacao = df_estacao.drop(index=[0]).reset_index(drop=True)
        
        # Percorre os poluentes esperados
        for pol in poluentes:
            
            # Verifica se o poluente está presente na estação
            if pol in df_estacao.columns:
                
                # Cria um DataFrame somente com DATETIME e POLUENTE
                df_pol = df_estacao[["DATETIME",pol]]
                
                # Obtém a unidade registrada na primeira linha do poluente
                df_pol['UNIDADE'] = df_pol[pol][0]
                
                # Remove a linha utilizada para identificar a unidade
                df_pol = df_pol.drop(index=[0]).reset_index(drop=True)
                # Mantém somente registros que possuem valores numéricos na coluna DATETIME
                df_pol = df_pol[df_pol["DATETIME"].astype(str).str.contains(r"\d", na=False)].reset_index(drop=True)
                # Aplica a função que corrige horários com 24:00
                df_pol['DATETIME'] = df_pol['DATETIME'].apply(fix_24h)
                
                # Padroniza o nome da coluna de concentração para VALOR
                df_pol = df_pol.rename(columns={pol:'VALOR'})
                ...
                # Substitui o nome do poluente pelo nome padronizado
                pol = dict_pols[pol]

                # Converte os valores de concentração para numérico
                df_pol['VALOR'] = pd.to_numeric(df_pol['VALOR'], errors='coerce')

                # Mantém somente as colunas padronizadas
                df_pol = df_pol[['DATETIME','ANO','MES','DIA','HORA','VALOR','UNIDADE','QAQC_INTERNO']] 
                # Armazena o DataFrame relacionando estação e poluente
    ...
    for chave in primeiros_valores.keys():
        ...
        if station in codigo_estacao_DF:
            # Se estiver e a data verificada for mais antiga que a registrada, atualiza a data
            if data <= codigo_estacao_DF[station]:
                codigo_estacao_DF[station] = data
        else: # Se não estiver, adiciona ao dicionário
            codigo_estacao_DF[station] = data
    ...
    for chave, df in dict_pols_stat.items():
        ...
        # Extrai o nome do poluente da chave e verifica seu código na tabela de códigos
        cod_pol = tabela_pols.loc[tabela_pols['POLUENTE'] == chave.split('_')[-1], 'COD_POLUENTE'].values[0]
...
# Salva o .csv       
    ...
    # Dicionário para substituir os nomes das estações pelos nomes padronizados
    dict_oema = {
...
def rectify_PE(path):
"""
A função padroniza os dados brutos de monitoramento de Pernambuco.
Lê os arquivos anuais, organiza os dados por estação e poluente,
padroniza as datas, garante a sequência horária, aplica o QAQC,
gera o ID_MMA das estações e salva os dados padronizados em arquivos CSV.
"""
    import glob
    import os
    import re
    ...
    # Cria um dicionário para armazenar os dados de todas as estações e poluentes encontrados nos diferentes arquivos/anos
    dict_pols_stat_global = defaultdict(lambda: pd.DataFrame())

    # Passa por cada arquivo Excel encontrado
    for arq in arquivos:

        if arq != '.ipynb_checkpoints':
            # Identifica o ano no nome do arquivo
            ...
            df = df.drop(index=[0, 1]).reset_index(drop=True)
            df = df.rename(columns={'Date Time': 'DATETIME'})
            # Define os nomes das colunas de poluentes presentes nos arquivos
            poluentes = ['CO_ppm', 'NO2_ug/m3', 'NO_ug/m3', 'NOx_ug/m3',
                         'O3_ug/m3', 'PM10', 'PM25', 'PTS', 'SO2_ug/m3']
            # Cria um dicionário padronizando os nomes dos poluentes
            dict_pols = {'CO_ppm': 'CO', 'NO2_ug/m3': 'NO2', 'NO_ug/m3': 'NO',
                         'NOx_ug/m3': 'NOX', 'O3_ug/m3': 'O3', 'PM10': 'MP10',
                         'PM25': 'MP25', 'PTS': 'PTS', 'SO2_ug/m3': 'SO2'}
            # Obtém os nomes das estações presentes no arquivo
            estacoes = set(df.columns[1:])
            # Cria um dicionário para armazenar os DataFrames de cada estação e poluente
            ...
            for estacao in estacoes:
                ...
                df_estacao.columns = [df_estacao.columns.tolist()[0]] + df_estacao.iloc[0, 1:].tolist()
                df_estacao = df_estacao.drop(index=[0]).reset_index(drop=True)
                # Verifica quais poluentes estão presentes na estação
                for pol in poluentes:
                    if pol in df_estacao.columns:
                        # Cria um DataFrame somente DATETIME e o poluente
                        df_pol = df_estacao[['DATETIME', pol]].copy()
                        # Obtém a unidade indicada na primeira linha do poluente
                        df_pol['UNIDADE'] = df_pol[pol].iloc[0]
                        # Remove a linha utilizada para armazenar a unidade
                        df_pol = df_pol.drop(index=[0]).reset_index(drop=True)
                        # Mantém somente registros que possuem informação de data/hora
                        df_pol = df_pol[df_pol['DATETIME'].astype(str).str.contains(r"\d", na=False)].reset_index(drop=True)
                        ...
                        # Renomeia a coluna do valor medido para VALOR
                        df_pol = df_pol.rename(columns={pol: 'VALOR'})
                        df_pol['VALOR'] = pd.to_numeric(df_pol['VALOR'], errors='coerce')
    
                        # ========= Garantia de sequência horária completa =========
                        # Define DATETIME como índice e ordena cronologicamente
                        df_pol = df_pol.set_index('DATETIME').sort_index()
                        
                        # Cria uma sequência contendo todas as horas entre o primeiro e o último registro
                        horas_completas = pd.date_range(df_pol.index.min(), df_pol.index.max(), freq='H')
                        # Reindexa o DataFrame para incluir as horas que não possuem registro
                        ...
                        # Cria a coluna QAQC_INTERNO:
                        # valores sem medição são considerados inválidos e os demais válidos
                        df_pol['QAQC_INTERNO'] = np.where(df_pol['VALOR'].isna(), 'Inválido', 'OK')
    
                        # Colunas auxiliares
                        # Cria ANO, MES, DIA e HORA a partir do índice DATETIME
                        df_pol['ANO'] = df_pol.index.year
                        df_pol['MES'] = df_pol.index.month
                        df_pol['DIA'] = df_pol.index.day
                        df_pol['HORA'] = df_pol.index.hour
                        # Retorna DATETIME para uma coluna e mantém somente as colunas utilizadas no padrão
                        df_pol = df_pol.reset_index().rename(columns={'index': 'DATETIME'})
                        df_pol = df_pol[['DATETIME', 'ANO', 'MES', 'DIA', 'HORA', 'VALOR', 'UNIDADE', 'QAQC_INTERNO']]

                        # Obtém a sigla padronizada do poluente
                        pol_sigla = dict_pols[pol]
                        # Cria uma chave no formato estação_poluente
                        chave = f"{estacao}_{pol_sigla}"
                        # Armazena o DataFrame da estação e poluente
                        dict_pols_stat[chave] = df_pol
    
            # ======== Acumula todos os anos (concatena) ========
            # Junta todos os dados do arquivo atual aos dados da mesma estação e poluente
    ...
    # Percorre cada estação_poluente e identifica a primeira data em que houve uma medição válida maior que zero
    ...
    # Percorre cada estação_poluente para aplicar o QAQC e salvar os dados
    for chave, df in dict_pols_stat_global.items():
        # Obtém o ID_MMA da estação
        estacao = codigo_estacao_PE.get(chave.split('_')[0], 'PE9999')
        # Extrai o nome do poluente da chave
        pol = chave.split('_')[-1]
        # Obtém o código do poluente na tabela de códigos
        cod_pol = tabela_pols.loc[tabela_pols['POLUENTE'] == pol, 'COD_POLUENTE'].values[0]
        # Obtém o nome da pasta do poluente a partir do código
        nome_pasta = tabela_pols.loc[tabela_pols['COD_POLUENTE'] == int(cod_pol), 'NOME_PASTA'].values[0]
        # Aplica o QAQC
        df = create_QAQCMMA_VALOR(df, nome_pasta)
        # Salva o .csv
...
def rectify_RR(path):
"""
A função padroniza os dados brutos de monitoramento de Roraima.
Separa os dados por estação e poluente, padroniza as datas, unidades e
informações de qualidade, aplica o QAQC e salva os dados em arquivos CSV.
Também cria um DataFrame relacionando os IDs das estações aos seus ID_MMA
"""    
    # 1. Configuração e Busca
    # Define o diretório onde estão os arquivos brutos
    target_file = "Medições (29).xlsx"
    path_dir = Path(path)
    # Lista os arquivos Excel existentes no diretório
    ...
    # Define o caminho principal onde os arquivos dos poluentes serão salvos
    ...
    # Define o ID_MMA de cada estação de acordo com o cadastro utilizado
    stations_map = {
    ...
    # O loop passa por cada arquivo Excel encontrado na pasta
    for item in files:
        ...
        col_tempo_key = None
        # Procura no cabeçalho MultiIndex a coluna que contém "Data e Hora"
        for c in df_multi.columns:
            if isinstance(c, tuple) and 'Data e Hora' in c[0]:
                col_tempo_key = c
                break
        # Caso a coluna não seja encontrada, utiliza a primeira coluna
        if col_tempo_key is None:
             col_tempo_key = df_multi.columns[0] 
        ...
        for est_nome, id_mma in stations_map.items():
            ...
            try:
                ...
                # Caso a estação não esteja presente no arquivo, passa para a próxima
                continue
            ...
            # O loop passa por cada poluente cadastrado e verifica se ele está presente na estação
            for pol_texto, pol_sigla in codigos_pols_RR.items():
                # Procura a coluna correspondente ao poluente
                col_match = [c for c in bloco.columns if pol_texto in c]
                # Se o poluente não estiver presente, passa para o próximo
                if not col_match: continue

                # Seleciona somente a coluna com os valores medidos, excluindo as colunas de flag, unidade e DATETIME
                value_col_name = [c for c in col_match if 'Flag' not in c and 'Unidade' not in c and c != 'DATETIME']
                # Caso não exista coluna de valor, passa para o próximo poluente
                if not value_col_name: continue
                value_col_name = value_col_name[0]

                # Procura a coluna de flag correspondente ao poluente
                col_flag = [
                    c for c in bloco.columns 
                    if ('Flag' in c or 'FLAG' in c) and pol_texto.split('(')[0].strip() in c[0]
                ]
                # Mantém somente DATETIME e o valor medido
                df_pol = bloco[["DATETIME", value_col_name]].copy()
                # Renomeia a coluna do valor medido para VALOR
                df_pol.rename(columns={value_col_name: 'VALOR'}, inplace=True)
                # Adiciona a flag original como QAQC_INTERNO
                df_pol['QAQC_INTERNO'] = bloco[col_flag[0]] if col_flag else np.nan
                # Informa caso não exista uma coluna de flag para o poluente
                if not col_flag:
                    print(f"⚠️ Nenhuma coluna de flag encontrada para {pol_texto} em {est_nome}")

                # Extrai a unidade informada no nome do poluente
                match_unit = re.search(r'\((.*?)\)', pol_texto)
                unit = match_unit.group(1).replace('µ', 'u') if match_unit else 'Nao declarado'
                # Padroniza a unidade removendo caracteres desnecessários
                df_pol['UNIDADE'] = unit.replace('[', '').replace(']', '')
                # Converte os valores medidos para formato numérico
                df_pol['VALOR'] = pd.to_numeric(df_pol['VALOR'], errors='coerce')
                # Remove datas inválidas, define DATETIME como índice e ordena cronologicamente
                ...
                # Preenche os horários sem registros com NaN
                ...
                # Obtém o nome da pasta do poluente a partir da tabela de códigos
                ...
                # Calcula a quantidade de registros válidos e inválidos após o QAQC
                total = len(df_pol)
                ...
                # Procura na tabela de IDs se já existe um ID_MMA_COMPLETO
                meta_match = tabela_ids.loc[(tabela_ids["ID_MMA"] == id_mma) & (tabela_ids["POLUENTE"] == pol_sigla), "ID_MMA_COMPLETO"]

                # Se existir, utiliza o ID_MMA_COMPLETO cadastrado
                if not meta_match.empty:
                     id_mma_completo = meta_match.iloc[0]
                else: # Caso contrário, cria o ID_MMA_COMPLETO a partir do ID da estação e do código do poluente
                ...
                # Cria a pasta do poluente caso ela ainda não exista
                pasta_destino.mkdir(parents=True, exist_ok=True)

                # Mantém somente as colunas padronizadas para o arquivo final
                ...
                df_final_save.to_csv(output_filepath, index=False)
                 ...
                 # Armazena as informações da estação: ID_OEMA, poluente, ID_MMA e ID_MMA_COMPLETO
                df_ids_list.append({
                    "ID_OEMA": est_nome,
                    "POLUENTE": pol_sigla,
                    "ID_MMA": id_mma,
...
def padronizar_horas(df, nome_df="df"):
    ...
    """
    # Renomeia a primeira coluna do DataFrame para DATETIME
    df = df.rename(columns={df.columns[0]: 'DATETIME'})
    # Converte a coluna DATETIME para o formato de data e hora do Pandas
    ...
    if df.empty:
        ...
        return df
    # Cria uma sequência com todas as horas esperadas entre a primeira e a última data
    full_range = pd.date_range(start=df['DATETIME'].min(), end=df['DATETIME'].max(), freq='h')
    # Define DATETIME como índice e preenche os horários que não possuem registros
    df = df.set_index('DATETIME').reindex(full_range).reset_index()
    # Renomeia o índice criado pelo reindex para DATETIME
...
def adicionar_colunas_data(df):
    ...
    """
    # Garante que DATETIME esteja no formato de data e hora do Pandas
    df['DATETIME'] = pd.to_datetime(df['DATETIME'], errors='coerce')
    # Cria as colunas de ano, mês, dia e hora a partir de DATETIME
    ...
    # Define as colunas de data que deverão ficar no início do DataFrame
    novas_colunas = ['DATETIME', 'ANO', 'MES', 'DIA', 'HORA']
    # Mantém as demais colunas após as colunas de data
...
def gerar_arquivos_estacoes_poluentes(dfs_alvo, UF_estacoes, df_cod_pol, pasta_saida):
    ...
    """
    # Percorre os nomes dos DataFrames que deverão ser processados
    for nome_var in dfs_alvo:
        # Verifica se o DataFrame existe no escopo global
        if nome_var not in globals():
            continue
        # Obtém uma cópia do DataFrame da estação
        df_estacao = globals()[nome_var].copy()
        # Extrai o ID_OEMA a partir do nome dos arquivos
        id_oema = nome_var.replace("df_", "").replace("_", " ").strip()

        # Buscar o ID_MMA correspondente ao ID_OEMA
        id_mma = UF_estacoes.loc[UF_estacoes['ID_OEMA'].str.lower() == id_oema.lower(), 'ID_MMA']

        # Verifica se foi encontrado um ID_MMA para a estação
        if id_mma.empty:
        ...
        # Considera como colunas de poluentes todas as colunas a partir da sexta coluna
        colunas_poluentes = df_estacao.columns[5:]
        
        # Percorre cada poluente encontrado na estação
        for col in colunas_poluentes:
            ...
            # Cria o nome do arquivo utilizando o ID_MMA e o código do poluente
...
def rectify_CE(path):
    ...
    for caminho in dados_brutos_CE:
        ...
        # Converte o nome do arquivo em um nome válido para uma variável Python
        ...
        with open(caminho, 'r', encoding='utf-8', errors='ignore') as f:
            amostra = f.read(2048)
            f.seek(0)
            try: # Identifica automaticamente o delimitador utilizado no arquivo
                delimitador = csv.Sniffer().sniff(amostra).delimiter
            except: # Utiliza vírgula caso o delimitador não seja identificado
                delimitador = ','

        # Ler CSV
        df_temp = pd.read_csv(caminho, sep=delimitador, dtype=str)
        
        # Cria o DataFrame no escopo global utilizando o nome do arquivo
        globals()[f"df_{nome_variavel}"] = df_temp
        
        # Armazena o nome do DataFrame na lista de DataFrames a serem processados
    ...
    for nome_var in dfs_alvo:
        # Obtém o DataFrame correspondente ao nome armazenado
        df = globals()[nome_var]

        # Padronizar horas
        df = padronizar_horas(df, nome_var)
        # Verifica se o DataFrame ficou vazio após a padronização
        if df.empty:
        ...
        # Percorre as colunas de poluentes para verificar se alguma precisa de conversão
        for col in colunas_poluentes:
            print(col)
            if col == 'CO(µg/m³)':
                print(col)

                # Cria um DataFrame temporário contendo o valor e a unidade original
                df_temp = pd.DataFrame({
                    "VALOR": pd.to_numeric(df[col], errors="coerce"),
                    "UNIDADE": "µg/m³"
                })

                # Converte a concentração de CO de µg/m³ para ppm
                df_temp = ug_to_ppm(df_temp)

                # Substitui os valores originais pelos valores convertidos
                df[col] = df_temp["VALOR"]
                # Atualiza a unidade para a unidade resultante da conversão
...
def rectify_PB(path):
"""
A função padroniza os dados brutos de monitoramento da Paraíba.
Lê os arquivos TXT das estações, padroniza os registros para uma
frequência horária, adiciona as informações de data e hora, aplica
o QAQC e gera os arquivos CSV padronizados por estação e poluente.
"""
    #--------------------------------------------------------
    # Caminhos
    #--------------------------------------------------------
    ...
    #--------------------------------------------------------
    # Carrega a tabela com a identificação das estações e a tabela com os códigos e nomes padronizados dos poluentes
    UF_estacoes = pd.read_csv(arquivo_estacoes, sep=',', dtype=str)
    ...
    # Ler dados brutos
    #--------------------------------------------------------
    # Localiza todos os arquivos TXT disponíveis na pasta de dados brutos
    arquivos_txt = glob.glob(os.path.join(pasta_dados, "*.txt"))

    # Define os nomes das colunas dos arquivos TXT.
    colunas = [
        "Data", "Serial", "Nome do Órgão",
        "PTS", "PM10", "PM25", "PM1", "Voltagem da Bateria"
    ]
    # Cria um dicionário para armazenar cada arquivo como um DataFrame
    dfs = {}
    # O loop passa por cada arquivo
    for arquivo in arquivos_txt:
        nome_base = os.path.splitext(os.path.basename(arquivo))[0]
        nome_var = "df_" + re.sub(r'\W+', '_', nome_base)

        try:
            # Lê o arquivo TXT utilizando ";" como separador e os nomes de
            # colunas definidos anteriormente
            df = pd.read_csv(arquivo, sep=';', header=None, names=colunas, encoding='utf-8')
            # Armazena o DataFrame utilizando o nome do arquivo como identificação
            dfs[nome_var] = df
    ...
    #--------------------------------------------------------
    # Padroniza a coluna DATETIME de cada estação e completa os horários
    # ausentes entre o primeiro e o último registro.
    ...
    #--------------------------------------------------------
    # Cria as colunas de ano, mês, dia e hora a partir do DATETIME
    for nome_var in dfs:
    ...
    #--------------------------------------------------------
    # Separa os dados por estação e poluente, associa os códigos padronizados  dos poluentes, aplica o QAQC e salva os arquivos finais na pasta MQAr.
...
def rectify_MS(path):
"""
A função padroniza os dados brutos de monitoramento do Mato Grosso.
Carrega os dados das estações e dos poluentes, combina os arquivos
brutos, organiza os registros por estação e poluente, cria uma
sequência horária contínua, aplica o QAQC.
"""
    import os
    import pandas as pd
    import numpy as np
...
# %% MAIN
# Dicionário que relaciona cada UF à função responsável por
# padronizar os dados brutos daquele estado.
funcoes = {
    'MG': rectify_MG, 
    'ES': rectify_ES,
    'SP': rectify_SP,
...
# Define quais estados serão processados nesta execução.
# Apenas os estados presentes nesta lista terão suas funções executadas.
lista_estados = ['MA']

# Carrega as tabelas gerais utilizadas pelas funções de padronização:
# Monitoramento_QAr_BR.csv contém as informações das estações e
# CODIGO_POLUENTES.csv contém os códigos e nomes padronizados dos poluentes.
...
# Percorre os estados selecionados e executa a função correspondente.
for estado in lista_estados:

    # Define o caminho da pasta de dados brutos daquele estado.
    path = os.getcwd()+'/data/DADOS_BRUTOS/' + estado + '/'

    # Executa a função de padronização correspondente ao estado.
