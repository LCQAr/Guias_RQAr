...
DESCRICOES_METRICAS = [
    ...
    (re.compile(r"^N_AUMENTO_REFERENCIA_(\d{4})_(\d{4})$"), "Aumento de estações de referência entre {1} e {0}"),
...
# dados para outras seções). Ajuste esta lista para os caminhos reais.
# --------------------------------------------------------------------------
SEM_VISUALIZACAO = {
...
# Seções que dependem dos scripts auxiliares rodando antes. Ajuste para
# os caminhos reais das suas seções 4.4.1 / 4.4.2.
DEPENDENCIAS = {
...
def rodar_notebook_com_dependencias(caminho_notebook: str, uf: str):
    ...
    with st.spinner(f"Rodando {nb_path.name} para {uf}..."):
...
with tab_secoes:
    ...
    if secao_selecionada in SEM_VISUALIZACAO:
        st.info(
            ...
            "por outras seções (ex: 4.4.1, 4.4.2). Rode-o aqui apenas se quiser "
            "atualizar esses dados manualmente. Pode demorar mais que o normal."
        )
        if rodar:
            with st.spinner(f"Rodando {notebook_path.name} para {uf_selecionada}... sem visualização disponível."):
                try:
        ...
        if gerado is None:
            ...
            st.warning(
                ...
                f"para **{gerado_uf}** — não bate com a seção/estado selecionados agora "
                f"({Path(secao_selecionada).name} / {uf_selecionada}). "
...
with tab_metricas:
    st.subheader(f"Métricas calculadas — {uf_selecionada}")
    st.caption("Mesmos números usados no relatório (compute_stats.py), calculados para o estado selecionado.")
 
    calcular = st.button("Calcular métricas", key="calcular_metricas")
    
    if calcular:
        from scripts import compute_stats as stats_module
        with st.spinner(f"Calculando estatísticas para {uf_selecionada}..."):
            try:
                ...
                st.error(f"Não foi possível calcular métricas para {uf_selecionada}: {e}")
    ...
    if metricas is None:
        ...
        st.warning(
            f"As métricas mostradas são de **{metricas_uf}** — clique em "
            f"'Calcular métricas' para atualizar para **{uf_selecionada}**."
        )
    else:
        # Números-chave em destaque
        col1, col2, col3, col4 = st.columns(4)
        ano = metricas.get("current_year", "")
        col1.metric(f"Total de estações no estado ({ano})", metricas.get(f"N_TOTAL_{ano}", "—"))
