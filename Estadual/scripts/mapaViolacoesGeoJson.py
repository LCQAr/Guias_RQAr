...
# Dados de entrada hospedados remotamente (nenhuma cópia local é necessária).
STATIONS_BASE_URL = "https://arquivos.lcqar.ufsc.br/data/databases/stations/"
META_URL = STATIONS_BASE_URL + "Monitoramento_QAr_BR.csv"
FASES_URL = STATIONS_BASE_URL + "fases_CONAMA506.csv"
AVERAGES_BASE_URL = STATIONS_BASE_URL + "MQAr_averages/"
...
# já que HTTP não suporta os.listdir()/Path.iterdir()/Path.glob() como um caminho local).
...
def mapa_violacoes_geojson(rootPath=None):
    ...
    # Caminhos principais
    # =======================
    # Os dados de entrada (MQAr_averages, Monitoramento_QAr_BR.csv, fases_CONAMA506.csv)
    # são lidos diretamente da URL hospedada; apenas a saída é local.
    ...
    for pol in poluentes:
        ...
        for ave_folder in ave_times_pol:
            ...
            pasta_url = f"{AVERAGES_BASE_URL}{ave_folder.replace('h', '')}/{pol}/"
            arquivos = list_remote_files(pasta_url, ext=".csv")
            if not arquivos:
                continue
            ...
            for nome_arq in arquivos:
                try:
                    df = pd.read_csv(pasta_url + nome_arq)
                ...
                if not col_valor:
                    print(f"Coluna de valor não encontrada em {nome_arq}")
                    continue
                    ...
                    for col in ["y1", "y0", "y1_val"]:
                        if col in fase and pd.notna(fase[col]):
                            ...
                            if val_num:
                                ...
                                break
                    if lim is None:
                        print(f"Limite não numérico para {pol} / {fase['phase']}")
                        continue
    ...
    # =======================
    if not registros:
        print("Nenhum registro válido encontrado — verifique se as pastas 1, 8 ou 24 contêm arquivos com colunas de data e valor.")
        return
