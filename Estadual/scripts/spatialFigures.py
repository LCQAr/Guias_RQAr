...
# Garanta que são números ANTES de criar o GeoDataFrame
    ...
    # Remova os NaNs que surgiram na conversão
    aqmData = aqmData.dropna(subset=["LATITUDE", "LONGITUDE"])
    ...
    # Fronteira do estado
    # -------------------------------------------------------------------------
    uf_boundary = geobr.read_state(code_state=UF, year=2020)  # geobr uses UF codes like 'SP', 'RJ' etc.

    # Plot the boundary first (as base), then add points on top
...
def spatial_rede_monitoramento_states(aqmData, UF, columnRef, columnsToltip, cmap):
    import geopandas as gpd
    import pandas as pd
...
# Garanta que são números ANTES de criar o GeoDataFrame
    ...
    m = gdf.explore(
        m=m,                     # <-- add points to the same map
        column=col_match,
        tooltip=tooltip_short,
        tooltip_kwds={"sticky": True, "direction": "right"},
        popup=popup_fields,
    ...
    # Zoom to the state boundary (better than just the points)
    minx, miny, maxx, maxy = uf_boundary.total_bounds
    m.fit_bounds([[miny, minx], [maxy, maxx]])

    MiniMap(position="bottomleft", zoom_level_offset=-5).add_to(m)

    # ... rest of your CSS macro code ...
