...
Ajuste este caminho para o diretório correspondente antes de executar o script. 
Como o código é compartilhado via Git, estes diretórios podem variar entre usuários. 
...
def columns_renamer_csv(mapping_df, aqmDisplay):
    """
...
def table_constructor(aqmData, columns_selector = ['FLAG','UF', 'Realiza monitoramento?','FONTE','REGIAO'],
...
def table01(aqmData, UF, path_bandeiras):
    """
    Generate an HTML-styled table summarizing air-quality monitoring sources by
    Brazilian states (UF).

    This function reads the dataset ``Monitoramento_QAr_BR.csv``, processes
    information on monitoring sources (``FONTE``) for each state, merges it with
    regional ordering metadata, inserts flag icons, groups states by region, and
    formats the output into a styled HTML table suitable for rendering in
    Jupyter notebooks.

    The resulting table contains:
        - State flag (HTML <img> tag)
        - UF code
        - Whether the UF performs any monitoring
        - Source(s) of monitoring data
        - The corresponding region

    Rows are grouped by region, with separating blank rows between groups to
    improve readability.
    ...
    IPython.display.HTML
        An HTML object that, when displayed, renders the fully styled table.

    Notes
    -----
    - Missing monitoring information is automatically labeled as "Não".
    - Missing data sources are labeled as "-".
    - Flag icons are expected in: ``../_static/bandeiras/{UF}.png``.
    - Uses external helper function ``tableReorder(regioes)`` to obtain
      ordering and region mappings.
    - Uses custom styling function ``style_all_white`` to format the table.
    ...
    # Atualizando o df com todos os estados
    aqmData = df_index.merge(aqmData, on='UF', how='inner')
    ...
    #Create a new column with HTML img tag
    aqmData['FLAG'] = aqmData['UF'].apply(
            lambda uf: f'<img src= "{path_bandeiras}{uf}.png" width="30">'
    ).astype(str)
    ...
    html_table = styled.to_html(index=False, border=0, escape=False)
    display(HTML(html_table))
    return html_table


def table05(aqmData, UF, path_bandeiras):
    ...
    aqmData = (
        aqmData
        .groupby('UF')['CATEGORIA']
        .value_counts()
        .unstack(fill_value=0)
    )
    
    # Garantir que todas as categorias esperadas existam,
    # mesmo quando não houver nenhuma estação nessa categoria
    categorias = ['Indicativa', 'Referencia', 'Nao declarado']
    
    for categoria in categorias:
        if categoria not in aqmData.columns:
            aqmData[categoria] = 0
    
    # Manter as colunas na ordem desejada
    aqmData = aqmData[categorias]
    
    # Total BR
    aqmData.loc['BR'] = aqmData.sum()
   
    df_index,uf_to_region,uf_order = tableReorder(regioes)
    # Keep only BR and selected UF
    df_index = df_index[df_index['UF'].isin([UF])]

    # Atualizando o df com todos os estados
    aqmData = df_index.merge(aqmData, on='UF', how='left')
    ...
    #Create a new column with HTML img tag
    # Alterar caminho para as bandeiras conforme necessário
    aqmData['FLAG'] = aqmData['UF'].apply(
        lambda uf: f'<img src= "{path_bandeiras}{uf}.png" width="30">'
    ).astype(str)
...
def table06MMA(aqmData, UF, path_bandeiras):
...
def table07(mapping_df, aqmData):
    ...
    df_with_separators= df_with_separators.replace(0, '-')
    df_with_separators = columns_renamer(mapping_df, df_with_separators)
    df_with_separators = columns_renamer_csv(mapping_df, df_with_separators)
...
def table07MMA(aqmData, UF, path_bandeiras, mapping_df):
    # Caminho para a pasta de dados
    # rootPath = os.path.dirname(os.getcwd())
    ...
    # Create a new column with HTML img tag
    aqmData['FLAG'] = aqmData['UF'].apply(
        lambda uf: f'<img src= "{path_bandeiras}{uf}.png" width="20">'
    ).astype(str)
...
def tabela_iterativa(aqmData, searchPaneColumns):
    ...
    # connected=True: embute o carregamento da biblioteca DataTables via CDN diretamente
    # no HTML retornado, tornando-o autossuficiente ao ser salvo como arquivo separado
    # (o show() abaixo usa connected=False por padrão, reaproveitando a biblioteca já
    # carregada uma única vez pelo init_notebook_mode() no próprio notebook).
    html_table = f"<style>{custom_style}</style>" + to_html_datatable(aqmData, connected=True, **table_kwargs)
    show(aqmData, **table_kwargs)

    return html_table


def flagTable(aqmData, UF, path_bandeiras, columnsSelector):
...
def flagTable2(aqmData, UF, path_bandeiras,columnsSelector):
...
def flagTable3(aqmData, path_bandeiras, columnsSelector):
    """
    Generate a formatted DataFrame of air quality monitoring stations with flags and pollutant counts.
    """
